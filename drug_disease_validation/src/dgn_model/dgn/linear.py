## Clone of the torch geometric module, but with configurable weight initialization
import copy
import math
import sys
import time
...
def reset_weight_(weight: Tensor, in_channels: int,
                  initializer: Optional[str] = None, uniform_bound: Optional[float] = None) -> Tensor:
    if in_channels <= 0:
        pass
    elif initializer == 'glorot':
        inits.glorot(weight)
    elif initializer == 'uniform':
        if uniform_bound is None:
...
class Linear(torch.nn.Module):
    ...
    def __init__(
        ...
        uniform_bound: Optional[float] = None,
    ):
        super().__init__()
    ...
    def reset_parameters(self):
        ...
        reset_weight_(self.weight, self.in_channels, self.weight_initializer, self.uniform_bound)
...
class HeteroLinear(torch.nn.Module):
    ...
    def reset_parameters(self):
        ...
        reset_weight_(self.weight, self.in_channels,
                      self.kwargs.get('weight_initializer', None), self.kwargs.get('uniform_bound', None))
